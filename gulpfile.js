...
gulp.task('compress', async () => {
  const world = require(path.join(GEO_DIR, '100', 'world.geo.json'));
  ...
  for (const size of sizes) {
    ...
    await pify(mapshaper.applyCommands)('-simplify dp ' + size + '%', world)
      .then(compressed => fs.outputJson(saveDir, compressed));
    bar.tick();
  }
});
